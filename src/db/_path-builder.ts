...
type Paths<T extends PathDefinitions> = {
    ...
    [K in keyof T]: T[K] extends { _type: 'staticDoc'; schema: infer S }
        ...
        ? // If it's a collection, the type is an intersection:
          ...
          // 2. Properties for the collection itself (e.g., .collectionPath).
          { collectionPath: string; collectionGroupPath: string; schema: S; converter: S extends z.ZodType ? DataConverter<z.infer<S>> : undefined; } &
...
export function createPaths<const T extends PathDefinitions>(definitions: T, parentSegments: string[] = []): Paths<T> {
    ...
    for (const [key, def] of Object.entries(definitions)) {
        ...
        if (def._type === 'staticDoc') {
            ...
            // Attach static properties like '.collectionPath'
            collectionBuilder.collectionPath = newSegments.join('/');
            collectionBuilder.collectionGroupPath = key;
