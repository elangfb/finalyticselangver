...
/**
 * Populates the two rows of KPI cards for Branch A and Branch B.
 */
export function populateBranchComparisonKPIs(periodData: SalesSummary[], branchA: string, branchB: string) {
  const calculateTotals = (summaries: SalesSummary[], branchName: string) => {
    const branchSummaries = summaries.filter((s) => s.branches.includes(branchName))
    const totals = branchSummaries.reduce((acc, s) => {
      acc.omzet += s.totalOmzet
      acc.checks += s.totalTransactions
      return acc
    }, { omzet: 0, checks: 0 });
    (totals as any).avgCheck = totals.checks > 0 ? totals.omzet / totals.checks : 0
    return totals
  }

  const totalsA = calculateTotals(periodData, branchA)
  const totalsB = calculateTotals(periodData, branchB)

  const updateDom = (periodSuffix: 'a' | 'b', data: typeof totalsA) => {
    const omzetEl = document.getElementById(`premium-branch-sales-total-omzet-${periodSuffix}`)
    const checkEl = document.getElementById(`premium-branch-sales-total-check-${periodSuffix}`)
    const avgCheckEl = document.getElementById(`premium-branch-sales-avg-check-${periodSuffix}`)
    if (omzetEl) omzetEl.textContent = formatCurrency(data.omzet)
    if (checkEl) checkEl.textContent = formatNumber(data.checks)
    if (avgCheckEl) avgCheckEl.textContent = formatCurrency((data as any).avgCheck)
  }

  updateDom('a', totalsA)
  updateDom('b', totalsB)

  const labelA = document.getElementById('premium-branch-sales-branch-a-label')
  const labelB = document.getElementById('premium-branch-sales-branch-b-label')
  if (labelA) labelA.textContent = branchA
  if (labelB) labelB.textContent = branchB
}

/**
 * Generates a flexible branch comparison chart for Omzet, TC, or APC.
 */
export function generatePremiumBranchComparisonChart(
  periodData: SalesSummary[],
  branchA: string,
  branchB: string,
  canvasId: string,
  metric: 'totalOmzet' | 'totalTransactions' | 'apc',
) {
  const daysInMonth = new Date(periodData[0]?.date.getFullYear(), periodData[0]?.date.getMonth() + 1, 0).getDate()
  const labels = Array.from({ length: daysInMonth }, (_, i) => String(i + 1))

  const getDailyData = (branchName: string) => {
    const dailyMap = new Map<string, number>()
    periodData.forEach((s) => {
      if (s.branches.includes(branchName)) {
        const day = String(s.date.getDate())
        dailyMap.set(day, (dailyMap.get(day) || 0) + s[metric])
      }
    })
    return labels.map((day) => dailyMap.get(day) || null)
  }

  const dataA = getDailyData(branchA)
  const dataB = getDailyData(branchB)

  const chartType = metric === 'totalTransactions' || metric === 'apc' ? 'bar' : 'line'
  const datasets: any[] = []

  if (metric === 'apc') {
    datasets.push({ type: 'line', label: `APC ${branchA}`, data: getDailyData(branchA), borderColor: '#A5B4FC', yAxisID: 'y-apc', tension: 0.1, spanGaps: true })
    datasets.push({ type: 'line', label: `APC ${branchB}`, data: getDailyData(branchB), borderColor: '#4F46E5', yAxisID: 'y-apc', tension: 0.1, spanGaps: true })
    datasets.push({ type: 'bar', label: `TC ${branchA}`, data: getDailyData(branchA), backgroundColor: '#A5B4FC', yAxisID: 'y-tc' })
    datasets.push({ type: 'bar', label: `TC ${branchB}`, data: getDailyData(branchB), backgroundColor: '#4F46E5', yAxisID: 'y-tc' })
  } else {
    datasets.push(
      { label: branchA, data: dataA, borderColor: '#A5B4FC', tension: 0.1, spanGaps: true },
      { label: branchB, data: dataB, borderColor: '#4F46E5', tension: 0.1, spanGaps: true },
    )
  }

  const options: any = metric === 'apc'
    ? {
        scales: {
          'y-tc': { type: 'linear', position: 'left', title: { display: true, text: 'Total Check (TC)' } },
          'y-apc': { type: 'linear', position: 'right', title: { display: true, text: 'Average Check (APC)' }, grid: { drawOnChartArea: false } },
        },
      }
    : { scales: { y: { beginAtZero: true, ticks: { callback: (n: any) => shortenCurrency(n) } } } }
  ...
  createChart(canvasId, 'line', { labels, datasets }, mergeChartOptions(
    options,
    chartTooltip({ label: (ctx: any) => `${ctx.dataset.label}: ${formatCurrency(ctx.parsed.y)}` }),
  ))
}
